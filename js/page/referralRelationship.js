$(function () {
    ...
    layui.use(['form','laydate', 'table', 'laypage'], function() {
        ...
        $("#getReferList").on("click",".det",function () {
            var zzId = $(this).attr("idz")
            var scId = $(this).attr("ids")
            var jsonParam = {
                upOrgId: zzId,
        ...
        var hosPage = {
            ...
            ,layout: ['prev', 'next']
            ,limit: 10  //得到每页显示的条数
            ,curr: pageN //得到当前页，以便向服务端请求对应页的数据。
            // ,jump: function(obj){
            //     console.log(obj)
            // }
        };
        ...
        function getHosList(jsonParams) {
            // if(getUrlParam("page") !== null){
            //     jsonParams.pageNo = getUrlParam("page")
            // }
                ...
                if(resultMsg.data.upOrgList == undefined){
                    ...
                    $.each(resultMsg.data.upOrgList, function (i,item) {
                        html += '<div class="bd_r refer_list">' +
                            ...
                            '<img src="../img/btn_delete.png" alt="" class="det" ids = "'+item.rflId+'" idz="'+item.id+'">' +
                            '</div>'
                        sjList.push(item.id)
                    })
                }
        ...
        $(".add_hospital").click(function () {
            ...
            layerZD = layer.open({
                ...
                btn2: function () {
                    ...
                    // })
                    // unique(hosId)
                    if(getUrlParam("hospitalId") !== null){
                        dqId = getUrlParam("hospitalId")
                    }
                    var jsonParamz = {
                        orgId: dqId,//当前医院ID
