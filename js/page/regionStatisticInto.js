$(function () {
    layui.use(['form','laydate'], function(){
        ...
        var colors = ['#0E97FE', '#F7AA2A'];
        var legData = ['转入','回转'];

        getCharts(jsonParam);
        function getCharts (jsonParam) {
            getAjax('/stat/statTurnInAndRotrayTime', jsonParam, function (resultMsg) {
                // 曲线图
                var lineData = resultMsg.data.timeXList;
                var xdataline = [];
                var lineinData = resultMsg.data.timeY1List;
                ...
                var lineCopt = chartOpt('转入统计',xdataline, colors, legData, lineser, 'line');
                lineC.setOption(lineCopt);
                // 饼图
                // var pieData = resultMsg.data.accFor;
                // var pieser = [
                //     {value: pieData.inNum, name: legData[0]},
                //     {value: pieData.rotNum, name: legData[1]},
                // ];
                var pieData = resultMsg.data.accForList;
                var pieser = [
                    {value: pieData[0].value, name: pieData[0].name},
                    {value: pieData[1].value, name: pieData[1].name},
                ];
                var pieCopt = pieOpt('转入&回转占比', colors, legData, pieser);
                ...
                // $.each(xdatabar, function (i, item) {
                //     // xdatabar.push(item.orgName);
                //     baroutData.push(item.inNum - 0);
                //     barrotData.push(item.rotNum - 0);
                // });
                var barser = [
                    {
                        name: legData[0],
                ...
                var barCopt = chartOpt('医院转入统计',xdatabar, colors, legData, barser, 'shadow', 30);
