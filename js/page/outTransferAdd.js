$(function () {
    window.onload = function(){
        window.frames['iframePho1'].contentWindow.postMessage('outTransferAdd1', '*');
        window.frames['iframePho2'].contentWindow.postMessage('outTransferAdd2,0', '*');
    }
    ...
    layui.use(['form','laydate', 'table', 'laypage'], function(){
            ...
            window.addEventListener('message', function(e) {
                ...
                if(e.data == 1000) {
                    ...
                    if (files[2] == 'outTransferAdd1') {
                        ...
                        window.frames['iframePho2'].contentWindow.postMessage('outTransferAdd2,' + fileObj.file2.length, '*');
                        $('#iframePho2').parent('.layui-input-block').append('<div><a target="_blank" href="' + files[0] + '">' +
                            '<img src="../img/pdf.png" alt="">' +
                            '<span>' + files[1] + '</span></a>' +
                            '<i class="iconfont delFile" type="2">&#xe716;</i></div>');
                        // if (fileObj.file2.length == 5) {
                        //     $('#iframePho2').hide();
                        // }
                    }
                }
        ...
        if (ids) {
            ...
            getAjax('/turnout/getTurnOutDetail',jsonParam,function (resultMsg) {
        ...
        form.verify({
            ...
            weightReg: function(value, item){ //value：表单的值、item：表单的DOM对象
                ...
                // if(!new RegExp("^\\d+(\\.\\d{1})?$").test(value)){
            ...
            valMaxLen: function (value, item) {
                ...
                if (item.name == 'zljg' && value.length > 2000) {
                    return '治疗经过字数不得超过2000字';
                }
            }
        });
        ...
        $('.mould-yy').click(function () {
            modelJson = {
                // orgId: $(this).attr('m-type'), // 机构标识
                // depId: $(this).attr('m-type'), // 科室标识
        ...
        function getAccOrg (orgId, vals) {
            ...
            getAjax('/sys/org/getRelHosOrg', {'id': orgId}, function (resultMsg) {
                if (resultMsg.data.upOrgList == undefined || resultMsg.data.upOrgList.length == 0) {
                    // guanxi.js函数
        ...
        form.on('submit(formSubmit)',function (data) {
            ...
            if (ids) {
                ...
                var accDoc = edisObj.accDocName;
                edisObj.imageFile = JSON.stringify(filelist);
                edisObj.id = ids;
                edisObj.status = 0;
                edisObj.accDocName = accDoc.split('-')[0];
                edisObj.accDocId = accDoc.split('-')[1];
                getAjax('/turnout/updateTurnOut', edisObj,function (resultMsg) {
                    window.location.href = 'outTransfer.html';
                });
            } else {
                var subObj = data.field;
                var accDoc = subObj.accDocName;
                subObj.accDocName = accDoc.split('-')[0];
                subObj.accDocId = accDoc.split('-')[1];
                subObj.imageFile = JSON.stringify(filelist);
                getAjax('/turnout/addTurnOut', subObj,function (resultMsg) {
                    window.location.href = 'outTransfer.html';
                });
            }
            return false;
